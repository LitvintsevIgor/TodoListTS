...
export const changeTaskTitleTC = (todolistId: string, taskId: string, newTitle: string) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
    ...
    if (findTask) {
        ...
        todolistsAPI.updateTask(todolistId, taskId, model)
            .then( res => {
                let updatedTaskTitle = res.data.data.item.title
                dispatch(changeTaskTitleAC(taskId, updatedTaskTitle, todolistId))
            } )
    }
